...
@export var point_reward: int = 90
@export var blood_color: Color = Color(0.2, 0.6, 0.2, 1.0)
...
const FIREBALL_SCENE := preload("res://scenes/weapons/ShamanFireball.tscn")

@onready var nav_agent: NavigationAgent3D = $NavigationAgent3D
@onready var _anim: AnimationPlayer = $Wizard/AnimationPlayer
...
func _play(anim: String) -> void:
	...
	if _anim.has_animation(anim):
		_anim.play(anim)
	elif anim == "Walk" and _anim.has_animation("Walking_A"):
		_anim.play("Walking_A")
...
func _play_oneshot(anim: String) -> void:
	...
	if _anim.has_animation(anim):
		_anim.play(anim)
	elif anim == "Cast" and _anim.has_animation("Spellcast_Shoot"):
		_anim.play("Spellcast_Shoot")
	elif anim == "Cast" and _anim.has_animation("1H_Melee_Attack_Chop"):
		_anim.play("1H_Melee_Attack_Chop")
	elif anim == "Death" and _anim.has_animation("Death_A"):
		_anim.play("Death_A")
	elif anim == "Death" and _anim.has_animation("Death_B"):
		_anim.play("Death_B")
...
func _physics_process(delta: float) -> void:
	...
	if dist <= attack_range and _has_line_of_sight(_player.global_position):
		velocity.x = move_toward(velocity.x, 0.0, move_speed)
		velocity.z = move_toward(velocity.z, 0.0, move_speed)
		_face_toward(_player.global_position)
		if _attack_timer <= 0.0:
			_attack()
		elif _attack_timer < attack_cooldown - CAST_LEN:
			_play("Idle")
	else:
...
func _update_barricade_attack() -> void:
	...
	if dist <= barricade_attack_range:
		...
		if _attack_timer <= 0.0:
			_attack_barricade()
		elif _attack_timer < barricade_attack_cooldown - CAST_LEN:
			_play("Idle")
	else:
		var next_point := nav_agent.get_next_path_position()
		var dir := next_point - global_position
...
func _attack() -> void:
	...
	get_tree().create_timer(CAST_LEN * 0.4).timeout.connect(
		func() -> void:
			...
			if _player == null or not is_instance_valid(_player):
				return
			var p := FIREBALL_SCENE.instantiate() as Node3D
			get_tree().current_scene.add_child(p)
			var shoot_pos := global_position + Vector3(0, 1.8, 0)
			p.global_position = shoot_pos
			var target_pos := _player.global_position + Vector3(0, 1.5, 0)
			p.setup((target_pos - shoot_pos).normalized(), attack_damage)
	)
